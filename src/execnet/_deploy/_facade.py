...
is who waits: under :mod:`execnet.trio` the caller's own loop already runs
it, and here the caller has no loop, so it goes to the host thread and the
...
def run_blocking(
    ...
    *args: Any,
    ...
    """Run ``async_fn(*args, targets)`` on the gateways' host, and wait.

    Every gateway has to be served by the same host: the driver is one
    ...
    one host -- so this is a clear error rather than a real limitation.
    """
    if not gateways:
    ...
    hosts = {id(session.host) for session in sessions}
    if len(hosts) > 1:
        raise ValueError(
            "all gateways must be served by the same execnet.Host: one driver"
            " task cannot reach channels belonging to another event loop."
            " Gateways from one Group always share a host."
        )

    from .._trio_host import host_call

    targets = targets_for(gateways)
    return host_call(  # type: ignore[no-any-return]
        sessions[0].host,
